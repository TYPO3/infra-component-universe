...
export class Universe extends LitElement {
  ...
  render() {
    ...
    for (let identifier of Object.keys(entries)) {
      ...
      items.push(html`
        <a class="universe-item universe-item--${entry.isButton ? 'button' : 'link'}${identifier === this.active ? ' universe-item--active' : ''}" href="${entry.href}">
      ...
      <div class="universe">
        <div class="universe-container">
            <ul class="universe-menu">
              ${items.map(item => html`<li class="universe-menu-item">${item}</li>`)}
            </ul>
        </div>
      </div>
      `;
